...
#pragma omp for schedule(static)
        for (size_t i = 0; i < variator_limit_pvt; i += 2) {
            pair<Node*, Node*> oo;
            if (selected_parents[i]->type == NodeType::Multi){
                auto *  one = new Multitree();
                auto *  two = new Multitree();
                for (size_t j = 0; j< ((Multitree * )selected_parents[i])->nodes.size(); j++) {
                    pair<Node*, Node*> stc = SubtreeVariator::SubtreeCrossover(*((Multitree * )selected_parents[i])->nodes[j], *((Multitree *)selected_parents[population_chunk_start_idx + randu() * population_chunk])->nodes[j], conf->uniform_depth_variation, conf->caching);
//                    if (ValidateOffspringSingleNode(stc.first, conf->maximum_tree_height, conf->maximum_solution_size) || ValidateOffspring(stc.second, conf->maximum_tree_height, conf->maximum_solution_size))
//                        break;
                    one->AppendSingleNode(stc.first);
                    two->AppendSingleNode(stc.second);
                }
                oo.first = one;
                oo.second = two;
            }
            else{oo = SubtreeVariator::SubtreeCrossover(*selected_parents[i], *selected_parents[population_chunk_start_idx + randu() * population_chunk], conf->uniform_depth_variation, conf->caching);}
...
#pragma omp for schedule(static)
        for (size_t i = offspring_size_pvt; i < variator_limit_pvt; i++) {
            while (offspring[i] == NULL) {
                if (selected_parents[i]->type == NodeType::Multi) {
                    auto *  m = new Multitree();
                    for (SingleNode * n: ((Multitree *) selected_parents[i])->nodes) {
                        m->AppendSingleNode(SubtreeVariator::SubtreeMutation(*n, *tree_initializer, conf->functions, conf->terminals, conf->initial_maximum_tree_height, conf->uniform_depth_variation, conf->caching));
                    }
                    offspring[i] = m;
                }
                else{offspring[i] = SubtreeVariator::SubtreeMutation(*selected_parents[i], *tree_initializer, conf->functions, conf->terminals, conf->initial_maximum_tree_height, conf->uniform_depth_variation, conf->caching);}
                if (!ValidateOffspring(offspring[i], conf->maximum_tree_height, conf->maximum_solution_size)) {
                    offspring[i]->ClearSubtree();
                    offspring[i] = NULL;
                    attempts_variation++;
        ...
        for (size_t i = offspring_size_pvt; i < variator_limit_pvt; i++) {
            while (offspring[i] == NULL) {
                if (selected_parents[i]->type == NodeType::Multi) {
                    auto *  m = new Multitree();
                    for (SingleNode * n: ((Multitree *) selected_parents[i])->nodes) {
                        m->AppendSingleNode(SubtreeVariator::SubtreeRDO(*n, conf->maximum_tree_height, fitness->TrainX, fitness->TrainY, *semlib, conf->uniform_depth_variation, conf->caching, conf->linear_scaling));
                    }
                    offspring[i] = m;
                }
                else{offspring[i] = SubtreeVariator::SubtreeRDO(*selected_parents[i], conf->maximum_tree_height, fitness->TrainX, fitness->TrainY, *semlib, conf->uniform_depth_variation, conf->caching, conf->linear_scaling);}
                if (!ValidateOffspring(offspring[i], conf->maximum_tree_height, conf->maximum_solution_size)) {
                    offspring[i]->ClearSubtree();
                    offspring[i] = NULL;
                    attempts_variation++;
...
#pragma omp parallel for schedule(static)
        for (size_t i = offspring_size_pvt; i < variator_limit_pvt; i += 2) {
            ...
            if (selected_parents[i]->type == NodeType::Multi){
                ...
                auto *  two = new Multitree();
                for (size_t j = 0; j< ((Multitree * )selected_parents[i])->nodes.size(); j++){
                    pair<Node*, Node*> stc =SubtreeVariator::SubtreeAGX(*((Multitree * )selected_parents[i])->nodes[j], *((Multitree *)selected_parents[population_chunk_start_idx + randu() * population_chunk])->nodes[j], conf->maximum_tree_height, fitness->TrainX, *semlib, conf->uniform_depth_variation, conf->caching, conf->linear_scaling);
                    one->AppendSingleNode(stc.first);
                    two->AppendSingleNode(stc.second);
                }
                oo.first = one;
            ...
            else{oo = SubtreeVariator::SubtreeAGX(*selected_parents[i], *selected_parents[randu() * selected_parents.size()], conf->maximum_tree_height, fitness->TrainX, *semlib, conf->uniform_depth_variation, conf->caching, conf->linear_scaling);}
            offspring[i] = oo.first;
            if (!ValidateOffspring(offspring[i], conf->maximum_tree_height, conf->maximum_solution_size)) {
                offspring[i]->ClearSubtree();
                offspring[i] = selected_parents[i]->CloneSubtree();
...
bool GenerationHandler::ValidateOffspring(Node* offspring, int max_height, int max_size) {
    if (offspring->type == NodeType::Multi) {
        for(SingleNode * n: ((Multitree *) offspring)->nodes) {
            if (!ValidateOffspringSingleNode(n, max_height,max_size))
                return false;
        }
        return true;
    }
    else{return ValidateOffspringSingleNode(offspring, max_height,max_size);}

}

bool GenerationHandler::ValidateOffspringSingleNode(Node *offspring, int max_height, int max_size) {
    if (max_height > -1) {
