...
Fitness * EvolutionState::FetchFitnessFunctionGivenProbName(string prob_name) {
    ...
    if (prob_name.compare("symbreg") == 0) {
        ...
        cout << "symbolic regression diversitfied error";
...
void EvolutionState::SetOptions(int argc, char* argv[]) {
    ...
    desc.add_options()
            ...
            ("diffTRAIN", po::value<string>(), "sets the set of data optimized for difference")
            ("diffTEST", po::value<string>(), "sets the set of data optimized for difference")
    ...
    if (vm.count("nrtrees")) {
        ...
        this->nr_trees = vm["nrtrees"].as<size_t>();
    ...
    if (config->semantic_variation) {
        if (config->nrtrees > 1){
            throw std::runtime_error("EvolutionState::SetOptions can not use multitree with semantic backpropagation");
        }
