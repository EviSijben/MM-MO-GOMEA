...
enum FOSType {
    FOSLinkageTree, FOSRandomTree, FOSUnivariate, FOSLinkageTreeIdv, FOSLinkageTreeIdvO
};
...
private:
    ...
    static std::vector<std::vector<size_t>> GenerateLinkageTreeFOS(size_t number_of_nodes, const std::vector<Node*>& population, arma::mat * MI_distribution_adjustments, bool fos_no_root_swap = false, bool idv_trees=true, bool multi_trees = false);
