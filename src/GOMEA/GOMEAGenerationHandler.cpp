...
//    }
    ...
    if (conf->gomea_replace_worst > 0) {
        ...
        for (size_t i = 0; i < conf->gomea_replace_worst * population.size(); i++) {
            ...
            population[ fit_order[ population.size() - 1 - i ] ] = tree_initializer->InitializeRandomTree(tis, conf->initial_maximum_tree_height, conf->functions, conf->terminals,conf->nrtrees);
